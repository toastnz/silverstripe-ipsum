 ...
 * Pinc
 */
class LoremIpsum extends DataExtension {
	...
	public function ipsum($type = 'words', $countOrWidth = 1, $tagsOrHeight = false, $array = false) {
		if ($type == 'words' || $type == 'word' || $type == 'heading') {
			return $this->words($countOrWidth, $tagsOrHeight, $array);
		} elseif ($type == 'paragraphs') {
			return $this->paragraphs($countOrWidth, 'p', $array);
		} elseif ($type == 'image') {
			if ($countOrWidth && $tagsOrHeight && $array) {
				return '<img src="http://placehold.it/' . $countOrWidth . 'x' . $tagsOrHeight . '"/>';
			} elseif ($countOrWidth && $tagsOrHeight) {
				return 'http://placehold.it/' . $countOrWidth . 'x' . $tagsOrHeight;
			} else {
				return 'http://placehold.it/' . $countOrWidth;
			}
		}
